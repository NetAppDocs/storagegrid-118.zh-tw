...
summary: 當S3用戶端更新擷取物件的中繼資料時、此訊息會由S3 API產生。如果中繼資料更新成功、伺服器就會發出此訊息。 
---
= SUBPD：S3中繼資料已更新
:allow-uri-read: 
:icons: font
...
[role="lead"]
當S3用戶端更新擷取物件的中繼資料時、此訊息會由S3 API產生。如果中繼資料更新成功、伺服器就會發出此訊息。
...
一致性控制標頭
 a| 
更新儲存區的法規遵循設定時、一致性控制HTTP要求標頭的值（若要求中有）。
...
版本ID
 a| 
更新中繼資料之物件的特定版本ID。對非版本化貯體中的貯體和物件的作業不包括此欄位。
