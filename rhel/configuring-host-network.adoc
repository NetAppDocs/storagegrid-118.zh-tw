...
.開始之前
* 您已檢閱 link:../network/index.html["網路連線準則StorageGRID"]。
* 您已檢閱相關資訊 link:node-container-migration-requirements.html["節點容器移轉需求"]。
...
將金鑰設定為「true」會導致容器使用主機NIC的MAC位址。此外、主機也會使用指定容器網路的MAC位址。根據預設、容器位址是隨機產生的位址、但如果您已使用設定一個位址 `_NETWORK_MAC` 節點組態金鑰、改用該位址。主機和容器將永遠擁有不同的MAC位址。
...
. 使用標準的 VLAN 介面命名慣例、建立使用此連結做為其相關「實體裝置」的 VLAN 介面 `physdev-name.VLAN ID`。
+
